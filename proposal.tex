...
\paragraph{Server specification and validator implementation}
...
\begin{align*}
  ...
  \sstep :\quad&Q\times C \times \Server \to \Server \\
...
\paragraph{Validity and acceptance}
...
A sound tester requires a {\em loose} validator that accepts all valid behavior,
and a faithful test harness that correctly interprets its observed behavior into
the validator's input; An exhaustive tester requires a {\em strict} validator
that rejects all invalid behavior, a faithful test harness, plus an exhaustive
test input generator that reveals all possible behavior of the SUT.  Here I
focus on the looseness and strictness of the validator.
