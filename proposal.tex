...
\lys{Notation from TaPL page 364 footnote.}
...
\lys{I first read soundness and completeness the opposite way, too.
  \textcite{Tretmans} defined ``soundness'', ``exhaustiveness'', and
  ``completeness'' for test suites (validator + test cases), where soundness
  says ``refinement implies acceptance'', exhaustiveness says ``acceptance
  implies refinement'', and completeness is ``sound and exhaustive''.  I
  remember similar definitions where ``soundness is no false reject'' in Mayur's
  class, so not sure if it's worth overriding these words' semantics.}
