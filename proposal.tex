...
\newcommand{\letin}[2]{\ottkw{let}\;#1=#2\;\ottkw{in}}
...
\newcommand{\nil}{\varepsilon}
\newcommand{\yld}{\ottkw{yields}}
\newcommand{\yields}[2]{#1\;\yld\;#2}
\newcommand{\accpt}{\ottkw{accepts}}
\newcommand{\accepts}[2]{#1\;\accpt\;#2}
\newcommand{\loose}{\ottkw{loose}}
\newcommand{\isloose}[2]{#1\;\loose_{#2}}
\newcommand{\strict}{\ottkw{strict}}
\newcommand{\isstrict}[2]{#1\;\strict_{#2}}
...
input-output.  This process is called {\em synchronous testing}.
...
My goal is to formalise the techniques in \cite{li2021modelbased} into a generic
...
guaranteeing the soundness and exhaustiveness of the validator logic.
...
Let $Q$ be the query type, $A$ be the response type, $S$ be the server state,
...
\begin{definition}[Trace validity]
  ...
  (Q\times A))$, then:
  \begin{enumerate}
  \item Any server can yield an empty trace:
  ...
  \item A server yields a non-empty trace if it can yield the head of the trace
    ...
    \begin{align*}
      &\forall s\in\Server,\forall q\in Q, \forall a\in A,\forall l\in\List(Q
      \times A), \\ &(\exists c\in C,\exists s'\in\Server \mid
      \sstep(q,c,s)=(a,s') \wedge \yields{s'}{l}) \\ &\implies
      \yields{s}{(q,a)+l}
    \end{align*}
  \end{enumerate}
\end{definition}
...
\begin{definition}[Trace acceptance]
  ...
  \begin{enumerate}
  \item Any validator accepts an empty trace:
  ...
  \item A validator accepts a non-empty trace if it can consume the head of the
    ...
    \begin{align*}
      &\forall v\in\Validator,\forall q\in Q, \forall a\in A,\forall
      l\in\List(Q\times A), \\ & (\exists v'\in\Validator\mid
      \vstep(q,a,v)=\Some{v'}\wedge\accepts{v'}{l}) \\ &\implies
      \accepts{v}{(q,a)+l}
    \end{align*}
  \end{enumerate}
\end{definition}

\paragraph{Looseness and strictness}
...
A sound tester requires a {\em loose} validator, and a faithful test harness
that correctly interprets its observed behavior into the validator's input; An
exhaustive tester requires a {\em strict} validator, a faithful test harness,
plus an exhaustive test input generator that reveals all possible behavior of
the SUT.  Here I focus on the looseness and strictness of the validator.

\begin{definition}[Looseness]
  A validator is {\em loose} per server specification if it accepts all valid
  trace of the specification:
  \[ \isloose{v}{s}\triangleq\forall l\in\List(Q\times A), \yields{s}{l}\implies\accepts{v}{l} \]
\end{definition}

\begin{definition}[Strictness]
  A validator is {\em strict} per server specification if it only accepts valid
  traces of the specification:
  \[ \isstrict{v}{s}\triangleq\forall l\in \List(Q\times A), \accepts{v}{l}\implies\yields{s}{l} \]
\end{definition}
