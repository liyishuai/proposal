...
This thesis is to present a systematic technique for testing networked systems.
...
My first formal specification of HTTP/1.1 was written as
QuickChick~\cite{quickchick} properties.  For the deterministic subset of HTTP,
the specification implements a reference server by translating the RFC
documents.  However, when specifying RFC~7232: Conditional
Requests~\cite{rfc7232}, various implementation of ETags should be allowed.  As
a result, the specification maintains a knowledge of the ETags for each
resource, and validates the server's behavior against this knowledge.

After specifying RFC~7232, I find this specification method non-scalable per
nondeterminism of the protocol, and propose specifying the protocol with
symbolic reference implementation that covers the space of nondeterministic
behavior.
...
\subsection{Practice in Model-Based Asynchronous Testing}
